...
const MenuItems = ({ isMobile, active, setActive, setIsOpen }) => {
  ...
  return (
    <ul className={`list-none flexCenter ${isMobile && 'flex-col h-full'}`}>
      {['Explore NFTs', 'MyNFTs'].map((item, i) => (
        <li
          key={i}
          onClick={() => {
            setActive(item);
...
const ButtonGroup = ({ setActive, router, setIsOpen }) => {
  ...
  return currentAccount ? (
    <Button
      ...
      handleClick={() => {
        setActive('/');
        router.push('https://americandragonfineart.com');
      }}
    />
  ) : (
    <Button
...
const Navbar = () => {
  ...
  return (
    <nav className="flexBetween w-full fixed z-10 p-4 flex-row border-b dark:bg-nft-dark bg-white dark:border-nft-black-1 border-nft-gray-1">
      <div>
        <Link href="/">
          <div className="flexCenter md:hidden cursor-pointer" onClick={() => { setActive('Explore NFTs'); }}>
            <Image src={images.logo02} objectFit="contain" width={42} height={42} alt="logo" />
      ...
      {/*
    theme === 'light' (If theme is equal to light)
    ? 'dark' (Then switch to dark)
    : 'light' (Else, set it to light)
     */}
